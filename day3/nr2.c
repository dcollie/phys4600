...
double newton_raphson(int p, double* c, double tolerance, double x0)
{
	double x = x0;
	...
	diffPolynomial(c,Dc,p);
	do
	{

		F = polynomial(x,p,c);		//F = cubic(x, 2.0, 0, 1.0, -3.0); 	//2.0 * pow(x,3) + x - 3.0;
		dF = polynomial(x,p-1,Dc); 	//dF = quadratic (x, 6.0, 0, 1.0);  //6.0 * x * x + 1.0;
		delta = - F / dF;
...
int main(int argc, char ** argv)
{
	...
	double x = newton_raphson(3, coeff3, 1e-9, 0);
